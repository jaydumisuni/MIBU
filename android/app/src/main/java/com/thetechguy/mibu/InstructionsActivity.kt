...
class InstructionsActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mibuPage("MIBU", "Instructions / THETECHGUY TOOL") {
            ...
            addView(mibuCard("Waiting", "Tap Start Waiting only after both fresh captures are ready and the timing window is close enough for them to remain fresh. The main screen shows one live countdown. Four timing-window lanes remain hidden and their state is available in Logs."))
            ...
            addView(mibuCard("Verification", "After MIBU reaches the four timing windows, use PC Helper to reboot to fastboot and continue with the official Mi Unlock Tool. MIBU does not claim request approval or unlock success by itself. Return to Mi Unlock Status binding only if Mi Unlock reports that the account/device is not added."))
